...
This skill finds and proves the cause. Landing the fix is `dev-workflow`'s job.
...
4. **Fix (only if asked).** Hand off to `dev-workflow`: the fix lands on an isolated worktree, the reproduction becomes an E2E-weighted regression test, and it goes through the normal validate → PR → CI flow.

## Boundaries

- Don't fix a bug you haven't reproduced. A green reproduction after the change is the evidence the fix worked.
- Don't inline worktree/commit/PR mechanics — that's `dev-workflow`.
