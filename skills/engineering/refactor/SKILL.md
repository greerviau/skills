...
Improve the structure of working code without changing its behavior — reduce duplication, clarify naming, simplify control flow, align with conventions. **Behavior preservation is the hard invariant**, and an unchanged, passing test suite is the proof. What counts as better structure is the vocabulary in `design`, not taste.
