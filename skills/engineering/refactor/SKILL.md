...
This is a deliberate, test-guarded pass you can point at any code, distinct from the one-shot `/simplify` on the current diff. Any change to observable behavior is out of scope — that's a feature (`dev-workflow`) or a fix (`debug`).
...
4. **Land via `dev-workflow`.** Isolated worktree, staged commits, and a PR description that makes the behavior-preserving guarantee explicit (so a reviewer checks *nothing changed*).
