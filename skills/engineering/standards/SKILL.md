...
- Decide that per comment rather than by feel. Name the fact the comment carries, then look for code on the lines it describes that already states it. The comment stays only when the fact is named and the code does not state it; an unnamed fact, a fact the code states, or a call you can't settle all delete.
