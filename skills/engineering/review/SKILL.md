...
3. **Standards pass**, run as a subagent in parallel with the spec-conformance pass (step 4). Check against the bar in the `standards` skill. Run the mechanical checks first:
   ...
   Then the judgment calls: structure judged against `design`'s vocabulary (module depth, information hiding, seam placement) rather than the cheapest path, whether the tests actually prove the behavior, and that the touched documentation surface is audited (`doc-audit`).
