...
Each category below separates user-invoked and model-invoked skills.
...
- **[wayfinder-starmap](skills/engineering/wayfinder-starmap/SKILL.md)** - renders one or more Wayfinder maps as an interactive browser star map with issue previews, status colors, and subsection filtering.
- **[triage](skills/engineering/triage/SKILL.md)** - classifies inbound GitHub issues and pull requests, then produces an agent-ready brief with evidence, scope, acceptance criteria, and verification.
- **[handoff](skills/engineering/handoff/SKILL.md)** - compacts the current conversation into a handoff document for another agent to continue.
...
- **[rfc](skills/engineering/rfc/SKILL.md)** - writes a design proposal that needs agreement before anyone implements it, against a bundled RFC template it carries so it works in a repo with none, applying the per-section rules (background before motivation, goals as end states, non-goals that link a real thing, alternatives with no verdict line, acceptance criteria that state an observable outcome) and taking an inline review round before the RFC is circulated.
